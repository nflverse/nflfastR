...
The `clean_pbp()` function does a lot of work cleaning up player names and IDs for the purpose of joining them to roster data. We recommend doing the join on `passer_id`, `rusher_id`, or `receiver_id`. Below we used `receiver_id` to figure out the position of targeted players, and then measure the top 5 at each position in terms of total EPA gained on pass targets.
