...
nflfastR::load_pbp(2020) %>%
  filter(week <= 17, complete_pass == 1 | incomplete_pass == 1 | interception == 1, !is.na(down)) %>%
...
nflfastR::load_pbp(2020) %>%
  filter(week <= 17, !is.na(down)) %>%
  group_by(fantasy_player_name, posteam) %>%
  summarize(
    yards = sum(yards_gained),
    tds = sum(touchdown == 1 & td_team == posteam),
...
Used `fixed_drive` and `fixed_drive_result` instead. See Example 4 above.
