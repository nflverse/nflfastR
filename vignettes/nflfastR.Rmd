...
So `r scales::percent(out$drive_score[1], accuracy = 0.1)` of 1st & 10 plays from teams' own 20 would see the drive end up in a score in 2003, compared to `r scales::percent(out$drive_score[2], accuracy = 0.1)` in 2015. This has implications for Expected Points models (see `vignette("nflfastR-models")`).
...
These match the official stats on NFL.com with the exception of Watson missing 22 passing yards (this is a bug related to laterals where he's missing yardage from the final play of the week 16 game against the Bengals).
...
nflfastR::load_pbp(2020) %>%
  ...
  dplyr::group_by(rusher_player_name, posteam) %>%
  dplyr::summarize(
    yards = sum(yards_gained),
    tds = sum(touchdown == 1 & td_team == posteam),
    att = dplyr::n()
