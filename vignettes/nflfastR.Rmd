...
In most cases, however, it is not necessary to use this function for individual games, because nflfastR provides both a [data repository](https://github.com/guga31bb/nflfastR-data) and two main play-by-play functions: `load_pbp()` and `update_db()`. We cover `load_pbp()` below, and please see [Example 8: Using the built-in database function] for how to work with the database function `update_db()`.

The easiest way to access the data in the data repository is the new function `load_pbp()`. It can load multiple seasons directly into memory and supports the efficient data format [`qs`](https://github.com/traversc/qs). Loading all play-by-play data of the 2018-2020 seasons is as easy as 
...
nflfastR::load_pbp(2020) %>%
  dplyr::filter(week <= 17, complete_pass == 1 | incomplete_pass == 1 | interception == 1, !is.na(down)) %>%
...
These match the official stats on NFL.com. Note that we're using `passing_yards` here because `yards_gained` is not equal to passing yards on plays with laterals.
...
data <- nflfastR::load_pbp(2020) %>%
  dplyr::filter(week <= 17, !is.na(down), !is.na(rusher_player_name))

rushes <- data %>%
  dplyr::group_by(rusher_player_name, rusher_player_id, posteam) %>%
  dplyr::summarize(
...
laterals <- data %>%
  ...
  dplyr::group_by(lateral_rusher_player_name, lateral_rusher_player_id) %>%
  dplyr::summarize(
  ...
  ungroup() %>%
  dplyr::rename(
    rusher_player_name = lateral_rusher_player_name,
    rusher_player_id = lateral_rusher_player_id
  )

rushes %>%
  dplyr::left_join(laterals, by = c("rusher_player_name", "rusher_player_id")) %>%
  dplyr::mutate(
    lateral_yards = dplyr::if_else(is.na(lateral_yards), 0, lateral_yards),
    lateral_att = dplyr::if_else(is.na(lateral_att), 0L, lateral_att),
    lateral_tds = dplyr::if_else(is.na(lateral_tds), 0L, lateral_tds)
    ) %>%
  dplyr::mutate(yards = yards + lateral_yards, att = att + lateral_att, tds = tds + lateral_tds) %>%
  dplyr::arrange(-yards) %>%
  dplyr::select(rusher_player_name, posteam, yards, att, tds) %>%
  utils::head(10) %>%
  knitr::kable(digits = 0)
```

This matches up exactly, though note the steps we had to go through to correctly account for yards gained on laterals (otherwise we would be missing 2 rushing yards from Chubb in the table above).

## Fantasy stats

What if we want total yards from scrimmage? Now we can use the `fantasy_player_name` column, which is the rusher on rush plays and receiver on receiving plays.

``` {r stats_fantasy}
nflfastR::load_pbp(2020) %>%
  dplyr::filter(week <= 17, !is.na(down)) %>%
...
Used `fixed_drive` and `fixed_drive_result` instead. See [Example 4: Using Drive Information].
