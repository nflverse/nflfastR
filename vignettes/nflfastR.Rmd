...
As discussed above, `nflfastR` has a data repository for old seasons, so there's no need to actually scrape them. Let's use that here (the below uses `load_pbp()`, but .csv and .parquet are also available).
...
pbp <- nflfastR::load_pbp(c(2003, 2020), qs = TRUE)
...
So `r scales::percent(out$drive_score[1], accuracy = 0.1)` of 1st & 10 plays from teams' own 20 would see the drive end up in a score in 2003, compared to `r scales::percent(out$drive_score[2], accuracy = 0.1)` in 2020. This has implications for Expected Points models (see `vignette("nflfastR-models")`).
