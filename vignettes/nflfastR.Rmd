...
These match the official stats on NFL.com (note the filter for `week <= 17` since official stats only count regular season games). Note that we're using `passing_yards` here because `yards_gained` is not equal to passing yards on plays with laterals.

While the above works, we've also provided a function that does this all for you: `get_player_stats()`. This function takes an `nflfastR` play-by-play dataframe as an input along with one other argument, `weekly`, which defaults to FALSE. When `weekly` is true, a week-by-week dataframe is returned (rather than an aggregate over the whole provided dataframe). Let's again replicate the top 10 players in passing yards:

``` {r stats2}
nflfastR::load_pbp(2020) %>%
  dplyr::filter(week <= 17) %>%
  nflfastR::get_player_stats() %>%
...
Again, this matches up exactly, though note the steps we had to go through to correctly account for yards gained on laterals (otherwise we would be missing 2 rushing yards from Chubb in the table above).

## Fantasy stats

What if we want total yards from scrimmage? The harder way is to use the `fantasy_player_name` column, which is the rusher on rush plays and receiver on receiving plays:
...
But we could also just use the `get_player_stats()` function again:
...
Please ask [in the Discord channel](https://discord.gg/5Er2FBnnQa).
