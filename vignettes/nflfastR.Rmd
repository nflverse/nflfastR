...
These match the official stats on NFL.com (note the filter for `season_type == "REG"` since official stats only count regular season games and in preparation for an 18th regular season game it's better not to use the `week` column). Note that we're using `passing_yards` here because `yards_gained` is not equal to passing yards on plays with laterals.
...
But we could also just use the `calculate_player_stats()` function again:
