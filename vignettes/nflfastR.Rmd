...
data <- nflfastR::load_pbp(2020) %>%
  dplyr::filter(week <= 17, !is.na(down), !is.na(rusher_player_name), play_type %in% c("run", "qb_kneel"))

rushes <- data %>%
  dplyr::group_by(rusher_player_id, posteam) %>%
  dplyr::summarize(
...
laterals <- data %>%
  ...
  dplyr::group_by(lateral_rusher_player_id) %>%
  dplyr::summarize(
...
rushes %>%
  dplyr::left_join(laterals, by = c("rusher_player_id")) %>%
  dplyr::mutate(
